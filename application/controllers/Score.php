...
class Score extends CI_Controller
{
    ...
    public function abstract_excel_2(){
        $object = new PHPExcel();
        ...
        for ($category = 0; $category <= 4; $category++) {
            // 카테고리명 출력
            $category_name = $this->getCategoryName($category); // 카테고리명 가져오기
            ...
            $start_row = $this->getCategoryRow($category) + 1; // 헤더 행 설정
    ...
    public function abstract_excel(){
        ...
        $object->setActiveSheetIndex(0)->setCellValue("A1", "Diabetes/Obesity/Lipid (clinical)");
        $object->getActiveSheet()->mergeCells('A1:E1');
        $object->setActiveSheetIndex(0)->setCellValue("A2", "");
    

        $table_columns_clinical = array("NO.", "초록번호", "발표자", "소속", "국적", "초록 제목", "전체 총합", "평균", "순위");
        $column = 0;
        foreach ($table_columns_clinical as $field) {
        ...
        $where_clinical = array(
            'type' => 0,
            'category' => 0
        );
        $list_clinical = $this->rating->get_abstract_excel($where_clinical);
    
        // clinical 데이터 채우기
        foreach ($list_clinical as $row) {
            $where_detail = array('idx' => $row['idx']);
            $detail_list = $this->rating->get_detail($where_detail); // 평균 얻기
            ...
            // 행 데이터 채우기
            $object->getActiveSheet()->setCellValueByColumnAndRow(0, $excel_row, $excel_row - 3);
            $object->getActiveSheet()->setCellValueByColumnAndRow(1, $excel_row, $row['submission_code']);
            $object->getActiveSheet()->setCellValueByColumnAndRow(2, $excel_row, $row['first_name']);
            $object->getActiveSheet()->setCellValueByColumnAndRow(3, $excel_row, $row['org']);
            $object->getActiveSheet()->setCellValueByColumnAndRow(4, $excel_row, $row['nation']);
            ...
            $object->getActiveSheet()->setCellValueByColumnAndRow(6, $excel_row, $row['total_sum']);
            $object->getActiveSheet()->setCellValueByColumnAndRow(7, $excel_row, $average);
            $object->getActiveSheet()->setCellValueByColumnAndRow(8, $excel_row, $excel_row - 3);
